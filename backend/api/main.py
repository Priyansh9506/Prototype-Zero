...
# ---- Initialize Roboflow Client (with API key from environment) ----
...
if settings.ROBOFLOW_API_KEY:
    try:
        ...
        logger.info("✓ Roboflow client initialized for image analysis")
    except ImportError:
        logger.warning("⚠️ inference-sdk not installed. Image analysis disabled.")
    except Exception as e:
        logger.error(f"⚠️ Failed to initialize Roboflow client: {e}")
else:
    logger.warning("⚠️ ROBOFLOW_API_KEY not set. Image analysis will be disabled.")
...
# In production, avoid using "*" for security
if settings.ENVIRONMENT == "production":
    cors_origins = [origin.strip() for origin in cors_origins if origin.strip()]
else:
    # In development, add common localhost variations
    cors_origins.extend(["http://127.0.0.1:3000", "localhost:3000"])
